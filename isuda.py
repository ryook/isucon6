...
_config = {
    ...
    'db_user':       os.environ.get('ISUDA_DB_USER', 'isucon'),
    'db_password':   os.environ.get('ISUDA_DB_PASSWORD', 'isucon'),
...
def sdbh():
    if hasattr(request, 'sdb'):
        return request.sdb
    else:
        request.sdb = MySQLdb.connect(**{
            'host': os.environ.get('ISUTAR_DB_HOST', 'localhost'),
            'port': int(os.environ.get('ISUTAR_DB_PORT', '3306')),
            'user': os.environ.get('ISUTAR_DB_USER', 'isucon'),
            'passwd': os.environ.get('ISUTAR_DB_PASSWORD', 'isucon'),
            'db': 'isutar',
            'charset': 'utf8mb4',
            'cursorclass': MySQLdb.cursors.DictCursor,
            'autocommit': True,
        })
        cur = request.sdb.cursor()
        cur.execute("SET SESSION sql_mode='TRADITIONAL,NO_AUTO_VALUE_ON_ZERO,ONLY_FULL_GROUP_BY'")
        cur.execute('SET NAMES utf8mb4')
        return request.sdb
...
def get_keyword(keyword):
    ...
    cur = dbh().cursor()
    cur.execute('SELECT description, keyword FROM entry WHERE keyword = %s', (keyword,))
    entry = cur.fetchone()
    if entry == None:
        abort(404)
